...
def _run_script(backend: str) -> str:
    return textwrap.dedent(
        ...
        $CASSIA_CMD validate markers.csv
        $CASSIA_CMD validate raw_markers.csv --celltype-column cluster --gene-column gene --n-genes 5
        ...
        $CASSIA_CMD subcluster run \\
          --markers subcluster_markers.csv \\
          --major-cluster-info "CD8 T cell in blood" \\
          --backend "$BACKEND" \\
          --dry-run \\
          --out runs/subcluster_dry_run

        echo "== Build consensus from example summaries =="
        $CASSIA_CMD consensus \\
          --inputs consensus_inputs/codex consensus_inputs/claude \\
          --out runs/example_consensus.csv
        """
    )
...
def _offline_script() -> str:
    return textwrap.dedent(
        ...
        PYTHON_BIN="${PYTHON_BIN:-python}"
        CASSIA_CMD="${CASSIA_CMD:-$PYTHON_BIN -m CASSIA.cli}"
        SHELL_AGENT="$PYTHON_BIN toy_agent.py {prompt_file}"
