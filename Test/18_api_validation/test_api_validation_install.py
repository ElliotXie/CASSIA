"""
CASSIA Test 18: API Key Validation Testing (PIP INSTALL MODE)
==============================================================
...
Usage:
    python test_api_validation_install.py
"""
...
def test_validation_basic():
    ...
    for provider in providers:
        ...
        try:
            start = time.time()
            is_valid = CASSIA.validate_api_keys(provider, verbose=True)
...
def test_caching_behavior():
    ...
    # Clear cache first
    CASSIA.clear_validation_cache(test_provider)

    # First validation (should make API call)
    print("\nFirst validation (should make API call)...")
    start1 = time.time()
    result1 = CASSIA.validate_api_keys(test_provider, verbose=True)
    ...
    print("\nSecond validation (should use cache)...")
    start2 = time.time()
    result2 = CASSIA.validate_api_keys(test_provider, verbose=True)
...
def test_force_revalidate():
    ...
    # First validation
    print("\nFirst validation...")
    CASSIA.validate_api_keys(test_provider, verbose=False)
    ...
    start = time.time()
    result = CASSIA.validate_api_keys(test_provider, force_revalidate=True, verbose=True)
    duration = time.time() - start
...
def test_invalid_key():
    ...
    try:
        result = CASSIA.validate_api_keys("openai", api_key=fake_key, verbose=True)
...
def test_missing_key():
    ...
    try:
        ...
        result = CASSIA.validate_api_keys("openai", verbose=True)
...
def test_thread_safety():
    ...
    def validate_thread():
        result = CASSIA.validate_api_keys(test_provider, verbose=False)
...
def run_api_validation_test():
    ...
    print_test_header("18 - API Key Validation Testing (PIP INSTALL MODE)")
    ...
    results_dir = results_dir_dict['base']
    ...
    metadata = create_test_metadata(
        test_name="api_validation_test_install",
        config=config,
        duration_seconds=duration,
        status=status,
        clusters_tested=[],
