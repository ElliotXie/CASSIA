...
run_provider_defaults_test <- function() {
  print_test_header("21 - Provider Defaults (R)")
  ...
  results <- create_results_dir("21_provider_defaults", get_test_mode())
  ...
  output_name <- file.path(results$outputs, "provider_defaults_test")
  ...
  tryCatch({
    ...
    output_items <- list.files(results$outputs, pattern = "^CASSIA_", full.names = TRUE)
