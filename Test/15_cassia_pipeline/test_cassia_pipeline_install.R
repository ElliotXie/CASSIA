# CASSIA Test 15: CASSIA Pipeline (R) - INSTALL MODE
# ===================================================
...
#
# Usage:
#     Rscript test_cassia_pipeline_install.R
...
run_cassia_pipeline_install_test <- function() {
  print_test_header("15 - CASSIA Pipeline (R) [INSTALL MODE]")
  ...
  tryCatch({
    log_msg("\nRunning runCASSIA_pipeline via R package (install mode)...")
    ...
    items <- list.dirs(results_dirs$outputs, recursive = FALSE, full.names = FALSE)
    cassia_dirs <- items[grepl("^CASSIA_", items)]
    if (length(cassia_dirs) > 0) {
      pipeline_output_dir <- file.path(results_dirs$outputs, cassia_dirs[1])
    }
  ...
  metadata <- create_test_metadata(
    test_name = "cassia_pipeline_install",
    config = config,
...
# Run test
success <- run_cassia_pipeline_install_test()
