...
unlink("C:/Users/ellio/AppData/Local/R/win-library/4.4/00LOCK-CASSIA", recursive = TRUE)
# Load libraries
library(reticulate)
library(CASSIA)

# Set working directory to this script's location (important for interactive use)
# This ensures results are saved in the test folder, not elsewhere
...
# Create output directory structure inside this folder
output_base <- "results"
output_dirs <- list(
    pipeline = file.path(output_base, "01_pipeline"),
    batch = file.path(output_base, "02_batch_analysis"),
    scoring = file.path(output_base, "03_scoring"),
    uncertainty = file.path(output_base, "04_uncertainty"),
    annotation_boost = file.path(output_base, "05_annotation_boost"),
    compare = file.path(output_base, "06_compare_celltypes"),
    subclustering = file.path(output_base, "07_subclustering"),
    additional_task = file.path(output_base, "08_annotation_boost_additional")
)
...
setLLMApiKey(Sys.getenv("OPENAI_API_KEY"), provider = "openai", persist = TRUE)
setLLMApiKey(Sys.getenv("ANTHROPIC_API_KEY"), provider = "anthropic", persist = TRUE)
setLLMApiKey("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", provider = "openrouter", persist = TRUE)
...
runCASSIA_pipeline(
  output_file_name = file.path(output_dirs$pipeline, "test_output"),
  tissue = "large intestine",
  species = "human",
  marker = markers_unprocessed,
  max_workers = 3,
  annotation_model = "google/gemini-2.5-flash-lite",
  annotation_provider = "openrouter",
  score_model = "google/gemini-2.5-flash-lite",
  score_provider = "openrouter",
  score_threshold = 98,
  annotationboost_model = "google/gemini-2.5-flash-lite",
...
output_name <- file.path(output_dirs$batch, "intestine_detailed")
...
quality_scores <- runCASSIA_score_batch(
  ...
  output_file = file.path(output_dirs$scoring, "intestine_detailed_scored.csv"),
  max_workers = 6,
  model = "google/gemini-2.5-flash-lite",
  provider = "openrouter"
)
...
runCASSIA_generate_score_report(
  csv_path = file.path(output_dirs$scoring, "intestine_detailed_scored.csv"),
  output_name = file.path(output_dirs$scoring, "intestine_detailed_report")
)
...
iteration_results <- runCASSIA_batch_n_times(
    n = 5,
    marker = markers_unprocessed,
    output_name = file.path(output_dirs$uncertainty, "intestine_Uncertainty"),
    model = "google/gemini-2.5-flash-lite",
    provider = "openrouter",
    tissue = "large intestine",
    species = "human",
...
similarity_scores <- runCASSIA_similarity_score_batch(
    marker = markers_unprocessed,
    file_pattern = file.path(output_dirs$uncertainty, "intestine_Uncertainty_*_full.csv"),
    output_name = file.path(output_dirs$uncertainty, "intestine_similarity"),
    max_workers = 6,
    model = "google/gemini-2.5-flash-lite",
    provider = "openrouter",
    main_weight = 0.5,
...
validation_results <- runCASSIA_annotationboost(
    ...
    marker = markers_unprocessed,
    output_name = file.path(output_dirs$annotation_boost, "monocyte_annotationboost"),
...
marker="IGLL5, IGLV6-57, JCHAIN, FAM92B, IGLC3, IGLC2, IGHV3-7, IGKC, TNFRSF17, IGHG1, AC026369.3, IGHV3-23, IGKV4-1, IGKV1-5, IGHA1, IGLV3-1, IGLV2-11, MYL2, MZB1, IGHG3, IGHV3-74, IGHM, ANKRD36BP2, AMPD1, IGKV3-20, IGHA2, DERL3, AC104699.1, LINC02362, AL391056.1, LILRB4, CCL3, BMP6, UBE2QL1, LINC00309, AL133467.1, GPRC5D, FCRL5, DNAAF1, AP002852.1, AC007569.1, CXorf21, RNU1-85P, U62317.4, TXNDC5, LINC02384, CCR10, BFSP2, APOBEC3A, AC106897.1"
  ...
  compareCelltypes(
      ...
      marker = marker,
      species = "human",
      output_file = file.path(output_dirs$compare, "plasma_cell_subtype")
    )
...
runCASSIA_subclusters(marker = marker_sub,
    ...
    output_name = file.path(output_dirs$subclustering, "subclustering_results"),
    model = "google/gemini-2.5-flash-lite",
    provider = "openrouter")
...
runCASSIA_subclusters(marker = marker_sub,
    ...
    output_name = file.path(output_dirs$subclustering, "subclustering_results2"),
    model = "google/gemini-2.5-flash-lite",
    provider = "openrouter")
...
runCASSIA_n_subcluster(n=5, marker_sub, "cd8 t cell",
                       file.path(output_dirs$subclustering, "subclustering_results_n"),
...
similarity_scores <- runCASSIA_similarity_score_batch(
    marker = marker_sub,
    file_pattern = file.path(output_dirs$subclustering, "subclustering_results_n_*.csv"),
    output_name = file.path(output_dirs$subclustering, "subclustering_uncertainty"),
    max_workers = 6,
    model = "google/gemini-2.5-flash-lite",
    provider = "openrouter",
    main_weight = 0.5,
...
runCASSIA_annotationboost_additional_task(
    full_result_path = paste0(output_name, "_full.csv"),
    marker = markers_unprocessed,
    output_name = file.path(output_dirs$additional_task, "T_cell_state"),
