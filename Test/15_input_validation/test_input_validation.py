"""
CASSIA Test 15: Input Validation
================================
...
def test_marker_list_validation():
    ...
    # Test 9: Ensembl IDs should raise error when auto_convert_ids=False
    try:
        validate_marker_list(
            ["ENSG00000141510", "ENSG00000134644", "ENSG00000087086"],
            auto_convert_ids=False  # Disable auto-conversion to test error behavior
        )
        print_test_result("Ensembl IDs raise error (auto_convert=False)", False)
        tests_failed += 1
    except MarkerValidationError as e:
        assert "Ensembl" in str(e), f"Wrong error message: {e}"
        print_test_result("Ensembl IDs raise error (auto_convert=False)", True)
        tests_passed += 1
    except Exception as e:
        print_test_result("Ensembl IDs raise error (auto_convert=False)", False)
        tests_failed += 1

    # Test 9b: Ensembl IDs with auto_convert=True should warn but continue (when mygene not installed)
    try:
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            result = validate_marker_list(
                ["ENSG00000141510", "CD4", "CD8", "FOXP3", "CD3", "CD19", "CD20", "CD56", "CD14", "HLA-DR"],
                auto_convert_ids=True
            )
            assert len(result) == 10
            # Should have warning about mygene not installed OR successful conversion message
            mygene_warnings = [x for x in w if "mygene" in str(x.message).lower() or "convert" in str(x.message).lower()]
            # At minimum, markers should be preserved
        print_test_result("Ensembl IDs with auto_convert=True", True)
        tests_passed += 1
    except Exception as e:
        print_test_result("Ensembl IDs with auto_convert=True", False)
        tests_failed += 1

    # Test 10: Entrez IDs (>30% numeric) should raise error when auto_convert_ids=False
    try:
        # 100% numeric - should error when auto_convert disabled
        validate_marker_list(
            ["7157", "675", "472"],
            auto_convert_ids=False
        )
        print_test_result("Entrez IDs raise error (auto_convert=False)", False)
        tests_failed += 1
    except MarkerValidationError as e:
        assert "numeric" in str(e).lower() or "Entrez" in str(e), f"Wrong error message: {e}"
        print_test_result("Entrez IDs raise error (auto_convert=False)", True)
        tests_passed += 1
    except Exception as e:
        print_test_result("Entrez IDs raise error (auto_convert=False)", False)
        tests_failed += 1
    ...
    try:
        with warnings.catch_warnings(record=True) as w:
            ...
            result = validate_marker_list(
                ["CD4", "CD8", "FOXP3", "CD3", "7157", "CD19", "CD20", "CD56", "CD14", "HLA-DR"],
                auto_convert_ids=False  # Use False to test original warning behavior
            )
            assert len(result) == 10
        ...
        print_test_result("Few numeric markers (<30%) warn but continue", False)
        tests_failed += 1
...
def run_all_tests():
    ...
    print_test_header("15 - Input Validation")
