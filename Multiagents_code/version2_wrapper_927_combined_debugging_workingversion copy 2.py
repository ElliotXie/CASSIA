...
def run_full_cell_type_analysis(df_path, output_json_name="cell_type_analysis_results.json", model="gpt-4", temperature=0, tissue="lung", species="human", additional_info=None):
    ...
    Only reject the annotation if multiple distinct cell types are strongly supported by several high-ranking markers. In cases of potential mixed populations, flag this for further investigation rather than outright rejection.
...
results = run_full_cell_type_analysis(
    ...
    output_json_name="prostate_results_broad2_debugging14.json",
    model="gpt-4o",
    temperature=0,
    tissue="Prostate",
    species="human",
