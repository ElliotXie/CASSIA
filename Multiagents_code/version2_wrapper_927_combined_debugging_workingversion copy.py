...
def run_full_cell_type_analysis(df_path, output_json_name="cell_type_analysis_results.json", model="gpt-4", temperature=0, tissue="lung", species="human", additional_info=None):
    ...
    Sometimes the single cell dataset will contain cell types that are not commonly seen in the tissue. As long as the celltype is consistent with the markers. It is still a valid annotation.
    ...
    Provide a confidence score (1-10) for the annotation
    ...
    for index, row in df.tail(2).iterrows():
        ...
        if result:
            results[broad_cell_type] = {
                ...
                "iterations": result.get("iterations", 1)  # Get iterations from result, default to 1 if not present
            }
...
results = run_full_cell_type_analysis(
    ...
    output_json_name="prostate_results_broad2_debugging13.json",
    model="gpt-4o",
    temperature=0,
    tissue="Prostate",
    species="human",
