...
def run_full_cell_type_analysis_tissue_blind(df_path, output_json_name="cell_type_analysis_results.json", model="gpt-4", temperature=0, species="human", additional_info=None):
    def run_cell_type_analysis(model, temperature, marker_list, species, additional_info):
        client = OpenAI()
...
Please provide an updated annotation addressing the validation feedback."""
    ...
    for index, row in df.iterrows():
        ...
        result, conversation_history = run_cell_type_analysis(model, temperature, marker_list, species, additional_info)
