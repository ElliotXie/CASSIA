...
import { Upload, CheckCircle, AlertCircle, X, Zap, Loader2, Download, ArrowLeft, Database, FileText, BarChart3 } from 'lucide-react';
import { useDropzone } from 'react-dropzone';
import { AgentModelSelector } from '@/components/AgentModelSelector';
import { CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { MODELS } from '@/lib/config/model-data';

export default function MergingClient() {
  ...
  // Test API key - lazy load llm_utils
  const handleTestApiKey = async () => {
    ...
    try {
      const { testApiKey } = await import('@/lib/cassia/llm_utils');
  ...
  // Process - lazy load mergingAnnotation
  const handleProcess = async () => {
    ...
    try {
      const { mergeAnnotations, mergeAnnotationsAll } = await import('@/lib/cassia/mergingAnnotation');
      let result;
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="text-xs text-gray-500 space-y-1">
                  <div className="flex items-center gap-1"><FileText className="h-3 w-3" /> Supported format: CSV</div>
                  <div className="flex items-center gap-1"><BarChart3 className="h-3 w-3" /> Expected columns: "True Cell Type", "Predicted Main Cell Type", "Predicted Sub Cell Types"</div>
                </div>
