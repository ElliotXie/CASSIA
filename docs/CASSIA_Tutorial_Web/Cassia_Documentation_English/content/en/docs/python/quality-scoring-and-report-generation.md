...
| `generate_report` | `True` | Whether to automatically generate HTML report |
| `reasoning` | `None` | Reasoning depth for GPT-5 series via OpenRouter only (`"low"`, `"medium"`, `"high"`). Python also accepts dict format: `{"effort": "high"}`. See [Reasoning Effort Parameter](setting-up-cassia.md#reasoning-effort-parameter). |

### Parameter Details

**Model Selection**
- Highly recommended: `openai/gpt-5.4` or `anthropic/claude-sonnet-4.6` for best accuracy
...
| `{output_file}_report.html` | Interactive HTML report with all CASSIA outputs (if `generate_report=True`) |
