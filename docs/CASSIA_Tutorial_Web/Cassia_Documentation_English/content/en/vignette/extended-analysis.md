...
This agent can be used after you finish the default CASSIA pipeline and are still unsure about a cell type. You can use this agent to get a more confident subtype annotation. Here we use the Plasma Cells cluster as an example to distinguish if it is more like a general plasma cell or other cell types. The models used by default are the thinking-based advanced models.
