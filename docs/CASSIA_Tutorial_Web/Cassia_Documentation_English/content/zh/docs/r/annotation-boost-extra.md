...
- **`model`** (字符串): 指定要使用的大型语言模型。推荐：`anthropic/claude-sonnet-4.5`。
- **`provider`** (字符串): 模型的提供商（例如，"openrouter"）。
- **`additional_task`** (字符串): 对簇执行的自定义分析任务（例如，"infer the state of this T cell cluster" 推断这个 T 细胞簇的状态）。
