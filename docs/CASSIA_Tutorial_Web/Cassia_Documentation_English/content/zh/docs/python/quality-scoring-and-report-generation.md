...
| `generate_report` | `True` | 是否自动生成 HTML 报告 |
| `reasoning` | `None` | 仅适用于通过 OpenRouter 使用的 GPT-5 系列模型的推理深度（`"low"`、`"medium"`、`"high"`）。Python 还接受字典格式：`{"effort": "high"}`。详见 [推理深度参数](setting-up-cassia.md#推理深度参数)。 |
...
| `{output_file}_report.html` | 包含所有 CASSIA 输出的交互式 HTML 报告（如果 `generate_report=True`） |
