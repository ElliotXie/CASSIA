...
class Agent:
    def __init__(self, system="", human_input_mode="never", provider="openai", model="gpt-4", temperature=0):
        self.system = system
        self.chat_histories = {}
    ...
    def __call__(self, message, other_agent_id):
        if other_agent_id not in self.chat_histories:
            ...
            if self.system and self.provider == "openai":
    ...
    def execute(self, other_agent_id):
        if self.provider == "openai":
            ...
            messages = [{"role": m["role"], "content": m["content"]} for m in self.chat_histories[other_agent_id]]
            ...
            return completion.content[0].text  # Corrected to access the text content properly
...
if __name__ == "__main__":
    ...
    # Allow user to select provider and model
    provider = input("Select provider (openai/anthropic): ").lower()
    if provider == "openai":
        model = input("Select OpenAI model (e.g., gpt-4, gpt-3.5-turbo): ")
    elif provider == "anthropic":
        model = input("Select Anthropic model (e.g., claude-3-sonnet-20240229): ")
    else:
        print("Invalid provider. Defaulting to OpenAI GPT-4.")
        provider = "openai"
        model = "gpt-4"

    # Update agents with user-selected provider and model
    final_annotation_agent.provider = provider
    final_annotation_agent.model = model
    coupling_validator_agent.provider = provider
    coupling_validator_agent.model = model
    formatting_agent.provider = provider
    formatting_agent.model = model
